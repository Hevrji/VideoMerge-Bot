...
async def videos_handler(bot: Client, m: Message):
    ...
    if os.path.exists(input_):
        await m.reply_text("Sorry Unknownian,\nAlready One in Progress!\nDon't Spam Plox.")
        return
    ...
    if isInGap is True:
        ...
        if (len(QueueDB.get(m.from_user.id)) >= 0) and (len(QueueDB.get(m.from_user.id)) <= Config.MAX_VIDEOS):
            ...
            if len(QueueDB.get(m.from_user.id)) == Config.MAX_VIDEOS:
                MessageText = "Okay Unknownian, Now Just Press **Merge Now** Button Plox!"
...
async def callback_handlers(bot: Client, cb: CallbackQuery):
    if "mergeNow" in cb.data:
        ...
        try:
            ...
            await cb.answer("Don't Spam Unknownian!", show_alert=True)
        ...
        try:
            ...
            await cb.answer("Your Queue Empty Unknownian!", show_alert=True)
        ...
        if (QueueDB.get(cb.from_user.id, None) is not None) or (QueueDB.get(cb.from_user.id) != []):
            ...
            await cb.answer("Sorry Unknownian, Your Queue is Empty!", show_alert=True)
        ...
        if (QueueDB.get(cb.from_user.id, None) is None) or (QueueDB.get(cb.from_user.id) == []):
            await cb.answer("Sorry Unknownian, Your Queue is Empty!", show_alert=True)
            return
        merged_vid_path = f"{Config.DOWN_PATH}/{str(cb.from_user.id)}/[@maheshgreat]_Merged.{FormtDB.get(cb.from_user.id).lower()}"
        if cb.data.split("_", 1)[-1] == "Yes":
            await cb.message.edit("Okay Unknownian,\nSend me new file name!")
            try:
